...
function TimeOption(props: TimeOptionProps) {
  ...
  if (timePickerModifying == 'start') {
    initialColor =
      option.hour == eventStart.hour && option.minute == eventStart.minute
        ? 'rgb(125, 189, 220)'
        : 'rgba(172, 170, 170, 1)';
  } else {
    initialColor =
      option.hour == eventEnd.hour && option.minute == eventEnd.minute
        ? 'rgb(125, 189, 220)'
        : 'rgba(172, 170, 170, 1)';
  }
  ...
  function handleClick(event) {
    ...
    if (timePickerModifying == 'start') {
      let newEventStart = eventStart.set({
        hour: option.hour,
        minute: option.minute,
      });
      setEventStart(newEventStart);
    } else {
      let newEventEnd = eventEnd.set({
        hour: option.hour,
        minute: option.minute,
      });
